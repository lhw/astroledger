...
<div class="mb-8">
	...
	{#if unseenPatches.length === 0}
		<EmptyState message="No new patches detected — scraper runs every 30 minutes." padding="p-5" />
	{:else}
		<div class="space-y-2">
			{#each unseenPatches as patch}
				<div class="sc-card p-4 flex flex-col gap-3 border-l-4 border-l-green-400">
					...
					{#if patch.related_markets.length > 0}
						<div class="border-t border-surface-200 pt-2">
							<p class="text-xs font-semibold uppercase tracking-wider text-amber-700 mb-1.5">Suggested for resolution</p>
							<ul class="space-y-1">
								{#each patch.related_markets as market}
									<li>
										<a href="/markets/{market.id}" class="text-xs text-surface-700 hover:text-primary-600 underline underline-offset-2">
											{market.title}
										</a>
									</li>
								{/each}
