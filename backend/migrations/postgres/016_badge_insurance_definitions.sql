...
-- created_at is TEXT so the Go layer can scan it into a string field directly.
...
-- ── Insurance Column ──────────────────────────────────────────────────────────
ALTER TABLE user_badges ADD COLUMN IF NOT EXISTS insurance TEXT NOT NULL DEFAULT '';

-- +goose Down

ALTER TABLE user_badges DROP COLUMN IF EXISTS insurance;
